# Cycle 08 — Apprentissage : Concepts et Critères
...
Le cycle Apprentissage est le huitième et dernier cycle de la pipeline fractale v4. Il ferme la boucle : toute énergie dépensée dans les cycles 1-7 doit produire un apprentissage explicite qui alimente le cycle Discovery suivant.
...
- **PDCA (Deming)** — Plan/Do/Check/Act : le cycle Apprentissage est la phase Check+Act du cycle complet de la pipeline.
- **Kolb** — Expérience concrète → Observation réflexive → Conceptualisation abstraite → Expérimentation active : les quatre étapes du sous-cycle fractal d'Apprentissage suivent cette progression.
- **Argyris — Double boucle** — Single-loop learning corrige les actions (ex : améliorer un test) ; double-loop learning remet en question les règles du jeu (ex : remettre en question la classification de risque elle-même). Le cycle Apprentissage vise systématiquement la double boucle.

Le résultat obligatoire du cycle est un **diff de harness** : quelles règles, quels seuils, quels standards ont été mis à jour en conséquence de ce cycle. Si aucune règle n'a changé, le cycle Apprentissage n'a pas été complété.
...
[01-Discovery] → [02-Cadrage] → [03-Conception] → [04-Build]
      ↑                                                  ↓
[08-Apprentissage] ←─────────────────────────── [05-Validation]
      │                                                  ↓
      └──── alimente ──────────────────→        [06-Release]
                                                         ↓
                                               [07-Run] ──→ signaux → [08]
...
Le cycle Apprentissage reçoit ses entrées de **Run** (métriques live, incidents, SLO) et de **Validation** (résultats de tests, quality gates, écarts découverts). Il transmet ses sorties vers **Discovery** (backlog enrichi, hypothèses reformulées, standards mis à jour).

**Position fractale** : chaque cycle (01 à 07) contient lui-même ses propres étapes Capitaliser et Transmettre (étapes 6 et 7 du sous-cycle universel). Le cycle 08 est la version amplifiée de ces deux étapes à l'échelle de la pipeline complète.
...
Le cycle Apprentissage peut démarrer quand **tous** les points suivants sont vrais :
...
### 4.3 Entrées conditionnelles (É/C uniquement)
...
Le cycle Apprentissage est **terminé** quand **tous** les points suivants sont vrais :
...
- L'amélioration est **continue**, pas saisonnière : chaque cycle, même T/F, traverse Capitaliser et Transmettre.
...
Mapping sur le sous-cycle fractal d'Apprentissage :
...
| RO — Observation réflexive | Définir + Concevoir | Rétrospective, postmortem, analyse des écarts |
| AC — Conceptualisation | Exécuter | Formuler des règles généralisables, mettre à jour les standards |
| AE — Expérimentation | Vérifier + Capitaliser + Transmettre | Décider les actions, les injecter dans Discovery |

Le style d'apprentissage dominant en développement logiciel est **Convergent** (AC + AE) — on préfère appliquer des concepts à des problèmes concrets. Le cycle Apprentissage doit donc toujours produire une action concrète testable (AE), pas seulement une réflexion (RO).
...
- Promotion de classe de risque imprévue (F → É ou É → C découvert en cours de cycle).
...
**DORA 2025 — findings clés pour l'Apprentissage** :
...
- Les 7 capabilities organisationnelles de l'IA incluent le travail en petits lots et le focus utilisateur — deux éléments directement mesurables en Apprentissage.

### 6.7 Knowledge Management — capture et diffusion du savoir

Le cycle Apprentissage produit trois niveaux de connaissance :
...
Le cycle Apprentissage traite uniquement les deux premières catégories. La troisième est un signal de double-loop learning défaillant.
...
Coût estimé de remboursement: T/F/M/É (classe de risque)
...
Le cycle Apprentissage est le **seul mécanisme officiel** par lequel les apprentissages du passé informent le futur. Sans cette connexion explicite, chaque Discovery repart de zéro.

Les **inputs Discovery** produits par Apprentissage sont de 4 types :
...
Le cycle Apprentissage est évalué sur les caractéristiques ISO 25010:2023 **du cycle lui-même** — pas du produit livré.

| Caractéristique ISO 25010:2023 | Critère pour le cycle Apprentissage | Seuil |
|---|---|---|
...
| **Interaction** | Le cycle Apprentissage est réalisable dans le temps alloué sans surcharge | Durée ≤ budget défini par classe de risque (§8) |
| **Safety** *(nouveau 2023)* | Aucune contrainte de risque identifiée n'est ignorée sans décision explicite | Zéro risque É/C non adressé |
...
Le sous-cycle fractal d'Apprentissage se module selon la classe de risque du cycle qui vient de se terminer.
...
| Classe | Budget Apprentissage |
|---|---|
| T | 5-15 min — review informelle, archivage minimal |
| F | 30-45 min — rétrospective courte, métriques archivées |
| M | 1-2 h — rétrospective formelle, analyse métriques, 1 input Discovery |
| É | 2-4 h — rétrospective complète, double-loop, revue ADR, update harness |
...
Chaque cycle de la pipeline suit le sous-cycle universel à 7 étapes. Voici son instanciation pour le cycle Apprentissage.
...
- Choisir le format de rétrospective selon le besoin du cycle :
  ...
  - **Double-loop matrix** (custom) : pour cycles É/C avec hypothèses à remettre en question.
...
Ces activités ne sont pas propres au cycle Apprentissage mais s'y appliquent avec une intensité particulière.

### 10.1 Observabilité de l'observabilité

Le cycle Apprentissage doit aussi évaluer la **qualité des données de monitoring** elles-mêmes :
...
La classification T/F/M/É/C est le pivot du système. Elle doit être auditée à chaque cycle É/C :
- Y a-t-il eu des promotions de classe imprévues pendant le cycle ? (F classé F mais comportait une migration — était É en réalité.)
...
En mode solo + agents IA, le cycle Apprentissage inclut une revue spécifique du comportement de l'agent :
...
Coûts à analyser en Apprentissage :
...
La dette d'accessibilité et d'i18n doit être trackée séparément de la dette technique générale car son coût de correction est exponentiel avec le temps (retrofitter coûte 5-10× plus cher que prévoir dès la conception). À chaque cycle Apprentissage :
...
| Métriques DORA archivées | `.planning/07-metrics/dora-metrics.md` | Tableau Markdown | ✅ (F+) |
| Registre dette mis à jour | `.planning/02-backlog/tech-debt/` | Fichiers par item | ✅ (F+) |
| Inputs Discovery | `.planning/02-backlog/` | PBI ou question ouverte | ✅ (F+, ≥ 1) |
...
| Rapport audit agent IA | `.planning/09-logs/agent-audit-cycle-<N>.md` | Markdown | ✅ (F+) |
...
### 12.1 Métriques de performance du cycle Apprentissage lui-même
...
| **Taux double-loop** | Questions double-loop posées / Rétrospectives | ≥ 1 par É/C | Par cycle É/C |
...
| **Inputs Discovery créés** | Inputs créés en Apprentissage | ≥ 1 par F+ | Chaque cycle |
| **Taux rejet agent** | Propositions rejetées / Propositions totales | ≥ 20 % | Chaque cycle |

### 12.2 Métriques de santé du système (archivées en Apprentissage)
...
| Aucun input Discovery produit sur 3 cycles | RED CARD pipeline | Revue de la connexion Apprentissage → Discovery |
...
| Apprentissage expérientiel | Kolb Experiential Learning Cycle (1984) | simplypsychology.org |
| Apprentissage organisationnel | Double-loop learning, Argyris & Schön (1978, 1996) | infed.org |
...
**Contexte** : Le rapport Discovery+Cadrage (§4.3) identifie cette question comme ouverte. Sans mémoire sémantique structurée, chaque cycle Apprentissage repart de zéro. Les 50 postmortems archivés n'informent pas le prochain Cadrage.
...
**Question** : Quand on découvre en cours de Build qu'un changement classé F est en réalité É, quel est le protocole exact ?
...
2. Action immédiate (pause de la PR, re-Discovery minimal, re-Conception partielle).
3. Traçabilité (log de promotion dans `.planning/05-changes/`).
4. Alimentation Apprentissage (la promotion déclenche une question de double-loop en rétrospective).

**Priorité** : Haute — à formaliser avant le premier cycle É de production.
...
**Priorité** : Faible — non bloquant, à traiter lors du premier audit annuel.
...
| 04 — Build | Quality gates mis à jour, règles agent mises à jour | Config CI, `.planning/agent/boundaries.yaml` |
...
Le cycle Apprentissage ne modifie **jamais** le périmètre ou les objectifs d'un cycle en cours. Il alimente exclusivement le prochain cycle Discovery. Toute modification urgente qui ne peut pas attendre Discovery passe par le protocole de promotion de classe (RED-CARD-A02) et le mécanisme de changement de scope (§7 du rapport Discovery+Cadrage — scope pivot).
...
|---|---|---|
| H1 — classification T/F/M/É/C suffisante | Validée / Invalidée / Ouverte | ... |
| ... | ... | ... |
...
**Classe de risque typique** : F/M/É
