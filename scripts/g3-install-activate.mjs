    ...
    if (Array.isArray(value)) {
      if (value.length === 0) {
        ...
        for (const item of value) {
          ...
          const safe = String(item).includes(':') || String(item).includes('#')
...
function main() {
  ...
  for (const { name, adapter, destPath, content } of plan) {
    ...
    if (isDryRun) {
      ...
      if (action === 'SKIP') {
        ...
        console.log(`  [${action}]${' '.repeat(Math.max(0, 20 - action.length))} ${adapter}/skills/${name}/SKILL.md`);
      }
    }
  ...
  console.log('');
  if (isDryRun) {
    console.log(`[G3-INSTALL] DRY-RUN COMPLETE — ${plan.length} planned writes across ${ADAPTERS.length} adapters`);
    console.log(`  Adapters : ${ADAPTERS.map(a => `${targetRoot}/${a}/skills/<name>/SKILL.md`).join('\n             ')}`);
  } else {
