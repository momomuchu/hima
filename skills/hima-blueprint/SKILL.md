...
lightweight decision record with rejected alternatives, so `hima-forge` never has to guess.
...
- The change touches ≥2 files, a schema/data-model shape, a public interface, or an
  architecture/library choice — i.e. any decision expensive to reverse after `hima-forge` starts.
...
- Component boundaries and contracts are explicit enough that `hima-forge` needs no further
  design judgment calls.
- Every material decision has a decision record with a rejected alternative.
- The blueprint references its source charter and is readable by `hima-forge` and `hima-verdict`.
