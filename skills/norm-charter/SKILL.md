...
cycle: no `norm-forge` entry is allowed without a sealed charter behind it.
...
- Seal the charter (mark it ready) only once scenarios, criteria, and criticality tags are all
  present; an unsealed charter blocks `norm-blueprint`/`norm-forge` entry.

## Done when

- Every acceptance criterion traces to a Gherkin scenario and carries a criticality tag.
- The charter is sealed and readable by `norm-blueprint` and `norm-forge` without reinterpretation.
