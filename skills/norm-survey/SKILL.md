...
single line of charter or code exists. norm-survey exists so that "what are we actually solving"
...
- Flag anything that looks like a true blocker for `norm-lookout` or `norm-parley`, not for
  silent assumption.
- Record the survey as a durable artifact the next stage (`norm-charter`) can read.

## Done when

- The problem topology, dependencies, and unknowns are written down, not just discussed.
- Candidate requirements exist in a form `norm-charter` can turn directly into Gherkin scenarios.
