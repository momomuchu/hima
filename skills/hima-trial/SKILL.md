...
integration, e2e as appropriate) and accumulate the evidence set that `hima-verdict` will check.
...
- A sealed charter exists and its scenarios need a corresponding test before (RED) or after
  (confirming GREEN) `hima-forge` writes code.
...
- Record the evidence set (which scenario → which test → pass/fail) so `hima-verdict` can audit
...
- The evidence set is written down and ready for `hima-verdict`'s independent check.
