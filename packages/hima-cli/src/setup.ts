/**
 * setup.ts — `hima setup` onboarding module.
 *
...
/**
 * mergeClaudeHooks — merge hima hook entries into a Claude settings object.
 *
 * MERGE semantics:
 ...
 *   When omitted the command is:   `hima hook <event> --format claude`
 */
...
const CODEX_MARKER =
  "# hima-codex-hooks (generated by `hima setup --runtime codex`)";
...
export async function runSetup(opts: SetupOpts): Promise<SetupResult> {
  ...
  if (runtime === "claude") {
    ...
    messages.push(
      `Hooks wired: ${settingsPath}` +
        ` — re-run "hima setup" at any time to refresh.`,
    );
  ...
  messages.push(
    `Setup complete for runtime "${runtime}".` +
      ` Verify hooks with: hima trace --root ${root}`,
  );
...
/**
 * Read the existing settings.json (if any), merge hima hooks, and write back.
 */
