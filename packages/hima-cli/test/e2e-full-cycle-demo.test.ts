 ...
 * direct @hima/core calls for the sealing itself — only read-only assertions afterward).
 *
 * Flow:
 *   1. `hima init --yes`                                 — scaffold .hima/config.json etc.
 *   2. `hima hook user-prompt-submit` with a "full" sigil — bootstraps a ward at "discovery"
 ...
 *   3. `hima hook stage-advance --stage <s> --status done-verified` for each of the 8
 ...
 *
 * GAP (reported, not hidden — do not fake a pass): the current `hima hook stage-advance`
 ...
 * hima-verdict's "never self-declared by the builder" rule is a skill-level/process convention,
 * not something `hima hook stage-advance` itself checks or blocks on.
 *
 * Pre-condition: `pnpm --filter @hima/cli build` (and its workspace deps) must have run.
 */
...
import { resumeWard, readTrace } from "@hima/core";
...
describe("(1) hima init --yes — scaffold the project", () => {
  it("exits 0", () => {
...
describe("(2) hima hook user-prompt-submit — ward bootstrap", () => {
...
describe("(3) hima hook stage-advance — seal all 8 stages as done-verified", () => {
...
describe("(4) trace shows every stage sealed with descriptive evidence", () => {
  ...
  // through `hima hook stage-advance` — there is no `--evidence` CLI flag today, so the only
