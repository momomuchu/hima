import {
  compareRiskClass as compareCanonicalRiskClass,
  type OperatingMode,
  RISK_CLASSES,
  type RiskClass,
  riskAtLeast,
} from "../types/canonical.js";
import { maxRiskClass } from "./risk-rank.js";
import type {
  Changeset,
...
const MANDATORY_ACTIVITIES: Record<RiskClass, string[]> = {
  ...
  M: [
    ...
    "rollback_plan",
    "active_slo_monitoring",
  ],
  H: [
    "problem_validation",
    "user_interviews_jtbd",
    "risk_classification",
    "formal_dor",
    "privacy_accessibility_impact",
    "adr",
...
export function classifyRisk(changeset: Changeset): ClassificationResult {
  const activeSignals = scanForForcingSignals(
    changeset.files,
    changeset.diffContent,
    changeset.labels,
    changeset,
  );
  const forcedClass = maxRiskClass(activeSignals.map((signal) => signal.forcedClass));
...
export function promoteRisk(
  ...
  if (compareRiskClass(target, current) <= 0) {
    throw new RiskClassificationError(
      "Promotion target must be higher than current class.",
      "INVALID_DEMOTION",
      {
        current,
        target,
...
export function demoteRisk(
  ...
  if (compareRiskClass(target, current) >= 0) {
    throw new RiskClassificationError(
      "Demotion target must be lower than current class.",
      "INVALID_DEMOTION",
      {
        current,
        target,
  ...
  if (activeSignals.some((signal) => riskAtLeast(signal.forcedClass, target))) {
    throw new RiskClassificationError(
      "Cannot demote while an incompatible forcing signal is active.",
      "FORCING_SIGNAL_ACTIVE",
      {
        activeSignals,
        target,
  ...
  if (!isAllowedDemotion(current, target)) {
    throw new RiskClassificationError(
      "Requested demotion is not allowed by the risk policy.",
      "UNAUTHORIZED_DEMOTION",
      {
        current,
        target,
  ...
  if (currentRank - targetRank > 1) {
    throw new RiskClassificationError(
      "Demotion cannot skip a risk class.",
      "CLASS_SKIP_NOT_ALLOWED",
      {
        current,
        target,
...
export function isBypassEligible(riskClass: RiskClass, changeset: Changeset): boolean {
  ...
  return (
    ...
    activeSignals.every((signal) => compareRiskClass(signal.forcedClass, "H") < 0) &&
...
export function getDeploymentStrategy(riskClass: RiskClass): DeploymentStrategy {
  switch (riskClass) {
    ...
    case "M":
      return "canary-10";
    case "H":
      return "canary-progressive";
    case "C":
      return "canary-with-flag";
  }
}
...
export function scanForForcingSignals(
  ...
  for (const file of files.map(normalizePath)) {
    if (CRITICAL_FILE_PATTERNS.some((pattern) => pattern.test(file))) {
      signals.push({ type: "file_path", value: file, forcedClass: "C" });
    } else if (HIGH_FILE_PATTERNS.some((pattern) => pattern.test(file))) {
      signals.push({ type: "file_path", value: file, forcedClass: "H" });
    }
  }

  for (const label of labels.map((label) => label.toLowerCase())) {
  ...
  for (const pattern of CRITICAL_DIFF_PATTERNS) {
    ...
    if (match?.[0]) {
      signals.push({ type: "diff_content", value: match[0], forcedClass: "C" });
    }
  }
  ...
  for (const pattern of HIGH_DIFF_PATTERNS) {
    const match = diffContent.match(pattern);
    if (match?.[0]) {
      signals.push({ type: "diff_content", value: match[0].trim(), forcedClass: "H" });
    }
  }
...
export function compareRiskClass(a: RiskClass, b: RiskClass): number {
  return compareCanonicalRiskClass(a, b);
}
...
function getImpactEstimate(changeset: Changeset): 1 | 2 | 3 | 4 | 5 {
  if (changeset.impactEstimate) {
    return changeset.impactEstimate;
  }
  ...
  switch (changeset.changeType) {
    ...
    case "infra":
      return 4;
    case "architecture_refactor":
      return 5;
  }
}

function getProbabilityEstimate(changeset: Changeset): 1 | 2 | 3 | 4 | 5 {
  if (changeset.probabilityEstimate) {
    return changeset.probabilityEstimate;
  }
...
function getStructuralMinimum(changeset: Changeset): RiskClass {
  if ((changeset.diffLinesNet ?? 0) > 300) {
    return "M";
  }
...
function buildJustification(
  ...
  structuralMinimum: RiskClass,
): string {
  if (activeSignals.length > 0) {
    const highestSignal = maxRiskClass(activeSignals.map((signal) => signal.forcedClass));
...
function hasSensitiveBypassPath(files: string[]): boolean {
  return files
    .map(normalizePath)
    .some((file) => HIGH_FILE_PATTERNS.some((pattern) => pattern.test(file)));
}

function isAllowedDemotion(current: RiskClass, target: RiskClass): boolean {
  if (current === "L" && target === "T") {
    return true;
  }

  if (current === "M" && target === "L") {
    return true;
  }

  if (current === "H" && target === "M") {
    return true;
  }

  if (current === "C" && target === "H") {
    return true;
  }
